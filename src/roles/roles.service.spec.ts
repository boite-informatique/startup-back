...
describe('RolesService', () => {
  ...
  describe('createRole', () => {
    it('should create a new role', async () => {
      const roleData: CreateRoleDto = { name: 'Admin', users: [1, 2], permissions: [3, 4] };
      const expectedResult = { id: 1, ...roleData };
      jest.spyOn(prismaService.role, 'create').mockResolvedValue(expectedResult);
      ...
      expect(prismaService.role.create).toHaveBeenCalledWith({
        data: {
          ...
          permissions: { connect: roleData.permissions.map((p) => ({ id: p })) },
        },
      });
  ...
  describe('findAllRoles', () => {
    it('should return all roles', async () => {
      const expectedResult = [{ id: 1, name: 'Admin', users: [], permissions: [] }];
      jest.spyOn(prismaService.role, 'findMany').mockResolvedValue(expectedResult);
  ...
  describe('findOneRole', () => {
    it('should return a role with a given id', async () => {
      const roleId = 1;
      const expectedResult = { id: roleId, name: 'Admin', users: [], permissions: [] };
      jest.spyOn(prismaService.role, 'findUnique').mockResolvedValue(expectedResult);
