...
export class TasksFinishedService {
    ...
    async create(body: CreateTaskFinishedDto) {
        return this.prismaService.taskFinished.create({
            data: {
    ...
    async find(task_id: number) {
        try {
            return await this.prismaService.taskFinished.findUnique({
                where: { task_id },
            });
        } catch (error) {
    ...
    async update(task_id: number, body: UpdateTaskFinishedDto) {
        try {
            return await this.prismaService.taskFinished.update({
                where: { task_id },
                data: body,
            });
        } catch (_) {
    ...
    async delete(task_id: number) {
        try {
            return await this.prismaService.taskFinished.delete({
                where: { task_id },
            });
        } catch (error) {
            throw new NotFoundException();
